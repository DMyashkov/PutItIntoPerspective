...
import modelsOg from "/lineup.json";
import { gsap } from "gsap";

window.onload = function () {
  const data = JSON.parse(localStorage.getItem("data"));
  const plasticHeight = localStorage.getItem("height");
  ...
  if (data && plasticHeight && country) {
    ...
    const plasticModel = {
      fileName: "trash_bag",
      name: `Estimated mismanaged waste`,
      height: parseFloat(`${heightt}`),
        ...
        if (modelAttribute.path == "cube") {
          ...
          loader.load(
            ...
            (gltf) => {
              ...
              console.log(
                `About to generate description for model ${modelAttribute.name}`,
              );
              generateDescription(scene, fontLoader, modelAttribute);
          ...
          modelAttributes[i].x =
            i > 0
              ? modelAttributes[i - 1].x +
                ...
                (modelAttributes[i].width + modelAttributes[i - 1].width) /
                  2 /
                  2
              : 0;
        }
        ...
        Promise.all(modelPromises)
          .then(() => {
            ...
            const cameraHopPositions =
              createCameraHopPositions(modelAttributes);
            camera.position.set(
